...
async function renderDashboard(id) {
  ...
  // Editable narrative blocks (Project Description, Sustainability Narrative).
  // The textarea is on-screen only; a print-only div renders the saved text so
  // the dashboard prints/exports as a clean report. Any team member with
  // project access can edit (saved via PUT /narrative on blur).
  const narrativeBlock = (field, title, placeholder, hint) => {
    const val = project[field] || "";
    return `
      <div class="dash-narrative">
        <div class="dash-section-title">${esc(title)}</div>
        <textarea class="dash-narrative-input no-print" data-narrative="${field}"
          maxlength="8000" rows="5" placeholder="${esc(placeholder)}">${esc(val)}</textarea>
        <div class="dash-narrative-hint no-print">${esc(hint)}<span class="dash-narrative-status" data-narrative-status="${field}"></span></div>
        ${val ? `<div class="dash-narrative-print">${esc(val).replace(/\n/g, "<br>")}</div>`
              : `<div class="dash-narrative-print dash-narrative-empty">—</div>`}
      </div>`;
  };
  ...
  view.innerHTML = `
    ...
    <div class="dash card">
      ...
      ${narrativeBlock("projectDescription", "Project Description",
        "Describe the project — scope, size, location, program, and goals.",
        "A short overview of the project. Saved automatically.")}
      ...
      ${narrativeBlock("sustainabilityNarrative", "Sustainability Narrative",
        "Tell the sustainability story — strategies pursued, priorities, trade-offs, and outcomes for this project.",
        "The project's sustainability approach in the team's own words. Saved automatically.")}

      <div class="report-footer">Generated by the PBK WSSP Tracker · ${today} · Required credits met: ${score.reqMet}/${score.reqTotal}</div>
    </div>
  `;

  // Auto-save narrative fields on blur (only when changed), mirroring the
  // credit-note save pattern. Updates the print-only view in place on success.
  view.querySelectorAll(".dash-narrative-input").forEach(ta => {
    ta.addEventListener("blur", async () => {
      const field = ta.dataset.narrative;
      const prev = project[field] || "";
      if (ta.value.trim() === prev) return;
      const statusEl = view.querySelector(`[data-narrative-status="${field}"]`);
      if (statusEl) statusEl.textContent = "Saving…";
      try {
        const saved = await api("PUT", `/api/projects/${id}/narrative`, { [field]: ta.value });
        project.sustainabilityNarrative = saved.sustainabilityNarrative || "";
        project.projectDescription = saved.projectDescription || "";
        const val = project[field] || "";
        const printEl = ta.parentElement.querySelector(".dash-narrative-print");
        if (printEl) {
          printEl.classList.toggle("dash-narrative-empty", !val);
          printEl.innerHTML = val ? esc(val).replace(/\n/g, "<br>") : "—";
        }
